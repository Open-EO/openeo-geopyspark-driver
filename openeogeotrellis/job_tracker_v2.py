...
class JobTracker:
    ...
    def _sync_job_status(
        ...
        if job_metadata.status in {
            ...
            # TODO: this `set_status` is just here to trigger the "mark_done from status" logic. Can this be automated even further?
            registry.set_status(
                job_id=job_id, user_id=user_id, status=job_metadata.status
            )
