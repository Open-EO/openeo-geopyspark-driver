...
class TestZooKeeperUserDefinedProcessRepository:
    ...
    @pytest.mark.parametrize(
        ["zk_client_reuse", "expected_create_call_counts", "expected_cleanups"],
        [
            (False, [0, 1, 2, 3], [3, 3]),
            (True, [0, 1, 1, 1], [0, 1]),
        ],
    )
    def test_client_reuse(self, kazoo_client_factory, zk_client_reuse, expected_create_call_counts, expected_cleanups):
        ...
        assert stop_call_counts == expected_cleanups
        assert close_call_counts == expected_cleanups
