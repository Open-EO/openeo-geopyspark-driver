"""
gdalinfo-based raster metadata for STAC assets.
"""
import copy
import json
import logging
...
from .result_metadata import convert_bbox_to_lat_long
from .util import GDALINFO_SUFFIX, make_set_for_key, parse_json_from_output

logger = logging.getLogger(__name__)


def poorly_log(message: str, *, job_id: Optional[str] = None, level=logging.INFO):
...
def exec_parallel_with_fallback(callback, argument_tuples, *, job_id: Optional[str] = None):
    callback_name = getattr(callback, "__name__", None) or getattr(callback, "func", callback).__name__

    def error_handler(e):
        poorly_log(f"Error while calling '{callback_name}', may be incomplete. {str(e)}", job_id=job_id, level=logging.WARNING)
...
def get_abs_path_of_asset(asset_filename: Union[str, Path], job_dir: Union[str, Path]) -> Path:
    ...
    if not abs_asset_path.is_absolute():
        abs_asset_path = Path(str(job_dir) + "/").absolute() / asset_filename

    return abs_asset_path


def resolve_asset_path(asset_href: str, job_dir: Path) -> Path:
    if asset_href.startswith("s3://"):
        assert job_dir.name in asset_href
        return get_abs_path_of_asset(asset_href[asset_href.rfind(job_dir.name) + len(job_dir.name) + 1 :], job_dir)
    else:
        return get_abs_path_of_asset(asset_href, job_dir)


def _get_metadata_callback(
    asset_md: Dict[str, str],
    job_dir: Path,
    asset_key: str,
    *,
    job_id: Optional[str],
    ...
    localize_asset: Callable[[str, Path], Optional[Path]],
):
    ...
    abs_asset_path: Path = resolve_asset_path(asset_href, job_dir)

    if not abs_asset_path.exists() and asset_href.startswith("s3://"):
        try:
            localized_path = localize_asset(asset_href, job_dir)
            if localized_path is not None:
                abs_asset_path = localized_path
            poorly_log(f"Downloaded {asset_href} to {abs_asset_path}", job_id=job_id, level=logging.DEBUG)
        except Exception as exc:
            message = (
                "Could not download asset from object storage: "
                + f"asset={abs_asset_path}, href={asset_href!r}, exception: {exc!r}"
            )
            poorly_log(message, job_id=job_id, level=logging.ERROR)

    asset_gdal_metadata: AssetRasterMetadata = read_gdal_raster_metadata(
        abs_asset_path, job_id=job_id, gdalinfo_from_file=gdalinfo_from_file, gdalinfo_use_subprocess=gdalinfo_use_subprocess
    )
...
def read_gdal_raster_metadata(
    ...
    *,
    ...
    """
    return parse_gdal_raster_metadata(
        read_gdal_info(
            str(asset_path), job_id=job_id, gdalinfo_from_file=gdalinfo_from_file, gdalinfo_use_subprocess=gdalinfo_use_subprocess
        ),
        job_id=job_id,
        gdalinfo_from_file=gdalinfo_from_file,
        gdalinfo_use_subprocess=gdalinfo_use_subprocess,
    )


def parse_gdal_raster_metadata(
    gdal_info: GDALInfo,
    *,
    job_id: Optional[str] = None,
    gdalinfo_from_file: bool = True,
    ...
    raster_md = _process_gdalinfo_for_netcdf_subdatasets(
        gdal_info, job_id=job_id, gdalinfo_from_file=gdalinfo_from_file, gdalinfo_use_subprocess=gdalinfo_use_subprocess
    )
    if raster_md:
        return raster_md
    else:
    ...
    *,
    ...
    for key, sub_ds_uri in gdal_info["metadata"]["SUBDATASETS"].items():
        if key.endswith("_NAME"):
            sub_ds_gdal_info = read_gdal_info(
                sub_ds_uri, job_id=job_id, gdalinfo_from_file=gdalinfo_from_file, gdalinfo_use_subprocess=gdalinfo_use_subprocess
            )
...
def read_gdal_info(
    ...
    *,
    ...
    if gdalinfo_use_subprocess or data_gdalinfo == {}:
        start = time.time()
        ...
        try:
            ...
            poorly_log(f"gdalinfo took {int((end - start) * 1000)}ms for {asset_uri}", job_id=job_id, level=logging.DEBUG)  # ~30ms
            if data_gdalinfo:
...
def _extract_gdal_asset_raster_metadata(
    ...
    *,
    ...
    callback = partial(
        ...
        gdalinfo_from_file=gdalinfo_from_file,
        gdalinfo_use_subprocess=gdalinfo_use_subprocess,
        localize_asset=localize_asset,
    )
    results = exec_parallel_with_fallback(callback, argument_tuples, job_id=job_id)
...
def extract_asset_metadata(
    ...
    *,
    ...
    :param epsg:
        ...
        the item level.)
    """
    raster_metadata, is_some_raster_md_missing = _extract_gdal_asset_raster_metadata(
        asset_metadata,
        job_dir,
        job_id=job_id,
        gdalinfo_from_file=gdalinfo_from_file,
