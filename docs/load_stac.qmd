...
backend implementation, which is why we define this best practice.

More broad [efforts are underway](https://github.com/EOEPCA/datacube-access/blob/main/best_practices/stac_best_practices.md)
to define best practices for STAC metadata. OpenEO seeks to align with these efforts as much as possible.


### Minimal STAC requirement summary

- Use STAC version 1.1 or higher
- The STAC resource passed to `load_stac` must allow discovery of available band names:
    - for an individual STAC item, the following places are consulted and first match is used:
        - using item property `eo:bands` (`eo` extension style) or `bands` (STAC 1.1 style)
        - bands discovered from visiting the linked parent collection (if available)
        - bands discovered from iterating over the assets included in the STAC item
          and merge the band info under their properties `eo:bands` (`eo` extension style) or `bands` (STAC 1.1 style)
    - for STAC collections, the following places are consulted and first match is used:
        - under the collection `summaries`,
          at field `eo:bands` (`eo` extension style) or `bands` (STAC 1.1 style)
        - under `item_assets`:
          iterate over the listed assets
          and combining the band info under their properties `eo:bands` (`eo` extension style) or `bands` (STAC 1.1 style)
        - (worst case):
          visiting the linked STAC items,
          extract band information as discussed above for individual items
          and merge the results
- The STAC assets that will be considered for loading:
    - have a supported media type (field `type`), including but not limited to:
      `image/tiff`, `image/vnd.stac.geotiff`, `image/jp2`, `application/netcdf`, `application/x-netcdf`
    - list the bands they contain under properties `eo:bands` (`eo` extension style) or `bands` (STAC 1.1 style)
    - have at least one role out of "data", "data-mask", "snow-ice", "land-water", "water-mask"
- Items have to include projection metadata (following the [STAC projection extension](https://github.com/stac-extensions/projection)):
    - `proj:code`
    - at least two out of `proj:bbox`, `proj:shape`, and `proj:transform` (make sure to double check the item order of `proj:transform` array)
