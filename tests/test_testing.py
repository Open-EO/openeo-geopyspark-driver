...
class TestDummyStacApiServer:
    ...
    def test_item_search_basic(self):
        with DummyStacApiServer().serve() as root_url:
            client = pystac_client.Client.open(root_url)
            result = client.search(method="GET", collections=["collection-123"])
            items = list(result.items())

        assert [item.id for item in items] == ["item-1", "item-2", "item-3"]

    def test_item_search_item_metadata(self):
        with DummyStacApiServer().serve() as root_url:
            client = pystac_client.Client.open(root_url)
            result = client.search(method="GET", collections=["collection-123"])
            items = list(result.items())
    ...
    def test_item_search_limit(self, limit, expected_items):
        with DummyStacApiServer().serve() as root_url:
            client = pystac_client.Client.open(root_url)
            result = client.search(method="GET", collections=["collection-123"], limit=limit)
            items = list(result.items())
    ...
    def test_item_search_datetime(self, search_datetime, expected_items):
        with DummyStacApiServer().serve() as root_url:
            client = pystac_client.Client.open(root_url)
            result = client.search(method="GET", collections=["collection-123"], datetime=search_datetime)
            items = list(result.items())
    ...
    def test_item_search_bbox(self, bbox, expected_items):
        with DummyStacApiServer().serve() as root_url:
            client = pystac_client.Client.open(root_url)
            result = client.search(method="GET", collections=["collection-123"], bbox=bbox)
            items = list(result.items())
