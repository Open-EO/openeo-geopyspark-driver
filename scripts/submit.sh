...
spark-submit --principal mep_tsviewer@VGT.VITO.BE --keytab mep_tsviewer.keytab --driver-memory 10G --executor-memory 5G --queue mep_tsviewer \
 ...
 --driver-class-path geotrellis-extensions-1.0.0.jar:geotrellis-backend-assembly-0.4.2.jar \
 --files layercatalog.json,jars/geotrellis-extensions-1.0.0.jar,dependencies/geopyspark/jars/geotrellis-backend-assembly-0.4.2.jar --conf spark.hadoop.security.authentication=kerberos --conf spark.yarn.maxAppAttempts=1 \
