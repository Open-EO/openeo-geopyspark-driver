...
def test_run_udf_code_records_execution_gauge_metrics(monkeypatch):
    ...
    assert captured_measurements[0][1] == {"openeo.udf.require_executor_context": False}
...
class TestInstallPythonUdfDependencies:
    ...
    def test_install_python_udf_dependencies_basic(self, tmp_path, dummy_pypi, caplog):
        ...
        assert "Successfully installed mehh-1.2.3" in caplog.text
    ...
    def test_install_python_udf_dependencies_fail(self, tmp_path, dummy_pypi, caplog):
        ...
        assert "ERROR: Could not find a version that satisfies the requirement nope-nope" in caplog.text
