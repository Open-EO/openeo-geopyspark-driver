"""
CDSE STAC API Assumptions Tests for Sentinel-3 OLCI LFR (sentinel-3-olci-2-lfr)

This test suite explicitly verifies the assumptions we make about the Copernicus
Dataspace STAC API for Sentinel-3 OLCI LFR collections (NRT and NTC).
...
Background:
- OL_2_LFR___ = OLCI Level-2 Full Resolution Land and atmosphere geophysical products
...
NRT_COLLECTION_URL = "https://stac.dataspace.copernicus.eu/v1/collections/sentinel-3-olci-2-lfr-nrt"
NTC_COLLECTION_URL = "https://stac.dataspace.copernicus.eu/v1/collections/sentinel-3-olci-2-lfr-ntc"
...
def test_assumption_both_collections_exist():
    ...
    assert nrt_collection["id"] == "sentinel-3-olci-2-lfr-nrt"
    ...
    assert ntc_collection["id"] == "sentinel-3-olci-2-lfr-ntc"
...
def test_assumption_product_type_is_same():
    """
    ASSUMPTION: Both NRT and NTC items have the same product:type value (OL_2_LFR___).
    ...
    # Query NRT
    nrt_data = query_stac("sentinel-3-olci-2-lfr-nrt", bbox, datetime_range, limit=3)
    ...
    # Query NTC
    ntc_data = query_stac("sentinel-3-olci-2-lfr-ntc", bbox, datetime_range, limit=3)
    ...
    # Both should have "OL_2_LFR___"
    assert "OL_2_LFR___" in nrt_product_types, "NRT should have OL_2_LFR___"
    assert "OL_2_LFR___" in ntc_product_types, "NTC should have OL_2_LFR___"
    _log.info("Both collections use product:type = 'OL_2_LFR___'")
...
def test_assumption_acquisition_key_uniqueness():
    ...
    nrt_data = query_stac("sentinel-3-olci-2-lfr-nrt", bbox, datetime_range, limit=10)
    ntc_data = query_stac("sentinel-3-olci-2-lfr-ntc", bbox, datetime_range, limit=10)
...
def test_assumption_timeliness_category_distinguishes_nrt_ntc():
    ...
    # Query NRT
    nrt_data = query_stac("sentinel-3-olci-2-lfr-nrt", bbox, datetime_range, limit=5)
    nrt_categories = {item["properties"].get("product:timeliness_category") for item in nrt_data.get("features", [])}

    # Query NTC
    ntc_data = query_stac("sentinel-3-olci-2-lfr-ntc", bbox, datetime_range, limit=5)
...
def test_assumption_same_platform_values():
    ...
    nrt_data = query_stac("sentinel-3-olci-2-lfr-nrt", bbox, datetime_range, limit=20)
    ntc_data = query_stac("sentinel-3-olci-2-lfr-ntc", bbox, datetime_range, limit=20)
...
def test_assumption_assets_have_hrefs():
    ...
    2. That asset href starts with "s3://eodata/Sentinel-3/OLCI/".
    """
    ...
    nrt_data = query_stac("sentinel-3-olci-2-lfr-nrt", bbox, datetime_range, limit=2)
    ...
    for item in nrt_data.get("features", []):
        ...
        for asset in item["assets"].values():
            href = asset.get("href", "")
            if ".SEN3" in href and href.startswith("s3://eodata/Sentinel-3/OLCI/"):
                found = True
...
def test_assumption_filename_contains_timeliness_marker():
    ...
    NRT products have "_NR_" marker, NTC products have "_NT_" marker.
    Example NRT: S3A_OL_2_LFR____20220101T000000_..._O_NR_003.SEN3
    Example NTC: S3A_OL_2_LFR____20220101T000000_..._O_NT_003.SEN3
    """
    ...
    for item in nrt_data.get("features", []):
        ...
        for asset in item["assets"].values():
            href = asset.get("href", "")
            if ".SEN3" in href:
                # Example: s3://eodata/.../S3A_OL_2_LFR____..._O_NR_003.SEN3/file.nc
                if "_NR_" in href:
                    _log.debug(f"  Found _NR_ in {href}")
                    break
        else:
            # If we got here, no asset had _NR_
            assert False, f"Expected to find _NR_ in at least one asset href for {item['id']}"

    _log.info("All NRT items have _NR_ marker in filename")
    ...
    for item in ntc_data.get("features", []):
        ...
        for asset in item["assets"].values():
            href = asset.get("href", "")
            if ".SEN3" in href:
                # Example: s3://eodata/.../S3A_OL_2_LFR____..._O_NT_003.SEN3/file.nc
                if "_NT_" in href:
