...
def test_resample__spatial_single_level(imagecollection_with_two_bands_and_three_dates):
    #print(imagecollection_with_two_bands_and_three_dates.pyramid.levels[0].to_spatial_layer(datetime.datetime(2017, 9, 25, 11, 37)).stitch())
    ...
    resampled.save_result("resampled.tiff", format="GTIFF")
    from osgeo.gdal import Info
    info = Info("resampled.tiff", format='json')
    print(info)
