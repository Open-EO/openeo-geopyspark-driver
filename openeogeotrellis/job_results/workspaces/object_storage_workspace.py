...
class ObjectStorageWorkspace(Workspace):
    ...
    def __init__(self, bucket: str, region: str, *, s3_client_factory: Callable[[str], "S3Client"]):
        self.bucket = bucket
        self.region = region
        self._s3_client_factory = s3_client_factory
        self._stac_io = CustomStacIO(s3_client_factory, region=region)
    ...
    def import_file(self, common_path: Union[str, Path], file: Path, merge: str, remove_original: bool = False) -> str:
        ...
        self._s3_client_factory(self.bucket).upload_file(
            str(file_absolute),
            self.bucket,
            key,
            Config=config,
    ...
    def import_object(self, common_path: str, s3_uri: str, merge: str, remove_original: bool = False) -> str:
        ...
        s3 = self._s3_client_factory(self.bucket)
