...
spark-submit --principal mep_tsviewer@VGT.VITO.BE --keytab mep_tsviewer.keytab --driver-memory 8G --executor-memory 5G --queue mep_tsviewer \
 ...
 --conf spark.locality.wait=300ms --conf spark.shuffle.service.enabled=true --conf spark.dynamicAllocation.enabled=true --conf spark.executor.extraClassPath=GeoPySparkExtensions-2.7.0-20180612.110004-15.jar:geotrellis-backend-assembly-0.4.0.jar \
 --driver-class-path GeoPySparkExtensions-2.7.0-20180612.110004-15.jar:geotrellis-backend-assembly-0.4.0.jar \
 --files layercatalog.json,../openeo-python-driver/jars/GeoPySparkExtensions-2.7.0-20180612.110004-15.jar,dependencies/geopyspark/jars/geotrellis-backend-assembly-0.4.0.jar --conf spark.hadoop.security.authentication=kerberos --conf spark.yarn.maxAppAttempts=1 \
