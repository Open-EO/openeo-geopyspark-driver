...
@pytest.mark.parametrize(["spatial_extent", "temporal_extent", "expected_shape"], [
    (
            ...
            ("2020-06-08T00:00:00", "2020-06-08T23:59:59"),
            (2, 1117, 1392),
    ),
...
def test_creodias_s1_backscatter(tmp_path, spatial_extent, temporal_extent, expected_shape):
    ...
    def some_polygons():
        with open("/opt/project/openeo-classification/src/openeo_classification/resources/training_data/crops_of_interest/sentinelhub/sampleable_polygons_year2019_zone35_id910091109120_p4.json") as f:
            return GeometryCollection([shape(feature["geometry"]) for feature in json.load(f)["features"]])
    polys = some_polygons()
    print(polys.bounds)
    load_params = LoadParameters(
        ...
        sar_backscatter=SarBackscatterArgs(
            ...
            elevation_model="off",
            options={"dem_zoom_level": 6,"implementation_version":"1","debug":True}
        ),
        aggregate_spatial_geometries=polys
    )
